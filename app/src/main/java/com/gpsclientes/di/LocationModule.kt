package com.gpsclientes.di

import android.content.Context
import com.google.android.gms.location.FusedLocationProviderClient
import com.google.android.gms.location.LocationServices
import com.gpsclientes.data.location.FusedLocationRepository
import com.gpsclientes.data.location.GeocoderGeocodingRepository
import com.gpsclientes.data.location.GeocodingRepository
import com.gpsclientes.data.location.LocationRepository
import dagger.Binds
import dagger.Module
import dagger.Provides
import dagger.hilt.InstallIn
import dagger.hilt.android.qualifiers.ApplicationContext
import dagger.hilt.components.SingletonComponent
import javax.inject.Singleton

@Module
@InstallIn(SingletonComponent::class)
abstract class LocationModule {

    @Binds
    abstract fun bindLocationRepository(
        impl: FusedLocationRepository
    ): LocationRepository

    @Binds
    abstract fun bindGeocodingRepository(
        impl: GeocoderGeocodingRepository
    ): GeocodingRepository

    companion object {
        @Provides
        @Singleton
        fun provideFusedClient(@ApplicationContext context: Context): FusedLocationProviderClient =
            LocationServices.getFusedLocationProviderClient(context)
    }
}
